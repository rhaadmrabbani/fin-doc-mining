...
def get_all_files(corpus_path, training_phase):
    dirs = [os.path.join(corpus_path, filename) for filename in os.listdir(corpus_path) \
            if os.path.isdir(os.path.join(corpus_path, filename)) and not filename.startswith('.') \
            and ( filename[ : 4 ] < '2014' if training_phase else filename[ : 4 ] >= '2014') ]
    return [ os.path.join(dir, filename) for dir in dirs for filename in os.listdir(dir) if not filename.startswith('.') ]
...
def load_data(corpus_path, training_phase, stem=True):
    ...
    files = get_all_files(corpus_path, training_phase)
...
def construct_corpus(corpus_path, training_phase, vocab_dict=None, threshold=5, topn=None, recursive=False):
    ...
    word_freq, doc_word_freq = load_data(corpus_path, training_phase)
    if training_phase:
...
def construct_train_test_corpus(in_path, output, threshold=5, topn=None):

    train_docs, vocab_dict, train_word_freq = construct_corpus(in_path, True, threshold=threshold, topn=topn, recursive=True)
    ...
    test_docs, _, _ = construct_corpus(in_path, False, vocab_dict=vocab_dict, recursive=True)
...
import re
label_re = re.compile( r'(?P<num>[iIl\d]+).*' )
label2_re = re.compile( r'[abcd]', re.I )
label_defect_re = re.compile( r'[iIl]' )

def generate_8k_doc_labels(doc_names, output):
    doc_labels = {}
    for each in doc_names:
        raw_label = each.split('_')[-1].replace('.txt', '')
        label = label_re.match(raw_label).group('num')
        label = label_defect_re.sub( '1', label )
        label = str(int(label) / 10) if int(label) >= 20 else label
        label2 = label2_re.search(raw_label)
        label2 = label2.group().upper() if label2 else ''
        doc_labels[each] = label + label2 if label in [ '1' , '7' ] else label
...
def generate_8k_bank_to_docs_map( doc_names , output ) :
    
    bank_doc_tuples = [ ( bank_re.search(doc).group('bank') , doc ) for doc in doc_names ]
    
    banks_to_docs_map = defaultdict( list )
    for bank , doc in bank_doc_tuples :
        banks_to_docs_map[ bank ].append( doc )
    
    dump_json(banks_to_docs_map, output)

    return banks_to_docs_map
