...
def train(args):
    ...
    dbow_train = doc_bow[train_idx]
    dbow_val = doc_bow[val_idx]
    del doc_bow

    start = timeit.default_timer()
    ldaskl = LatentDirichletAllocation(n_topics=args.n_topics, max_iter=args.n_iter,
                                       learning_method='online', learning_offset=50.,
                                       random_state=0).fit(dbow_train)
    joblib.dump(ldaskl, args.save_model)
    print 'runtime: %ss' % (timeit.default_timer() - start)

    if args.output:
        doc_keys = np.array(doc_keys)
        generate_doc_codes(ldaskl, doc_keys[train_idx], dbow_train, args.output)
        generate_doc_codes(ldaskl, doc_keys[val_idx], dbow_val, args.output + '.val')
...
def test(args):
    ...
    ldaskl = joblib.load(args.load_model)
    generate_doc_codes(ldaskl, doc_keys, doc_bow, args.output)
    print 'Saved doc codes file to %s' % args.output

    if args.save_topics:
        topics_prob = show_topics_prob(ldaskl, vocab_dict)
