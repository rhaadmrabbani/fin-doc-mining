...
def train(args):
    ...
    nmf = NMF(n_components=args.n_topics, max_iter=args.n_iter, random_state=1, alpha=.1, l1_ratio=.5).fit(dbow_train)
    joblib.dump(nmf, args.save_model)
    print 'runtime: %ss' % (timeit.default_timer() - start)

    if args.output:
        doc_keys = np.array(doc_keys)
        generate_doc_codes(nmf, doc_keys[train_idx], dbow_train, args.output)
        generate_doc_codes(nmf, doc_keys[val_idx], dbow_val, args.output + '.val')
...
def test(args):
    ...
    nmf = joblib.load(args.load_model)
    generate_doc_codes(nmf, doc_keys, doc_bow, args.output)
    print 'Saved doc codes file to %s' % args.output

    if args.save_topics:
        topics_prob = show_topics_prob(nmf, vocab_dict)
