...
def adjust_label(label):
    # 10-K
    return label if label in ['1','1A','7','7A'] else label_re.match(label).group()
...
def main():
    ...
    Y_train = [adjust_label(train_doc_labels[i]) for i in train_doc_codes]
    X_val = np.r_[val_doc_codes.values()]
    Y_val = [adjust_label(val_doc_labels[i]) for i in val_doc_codes]
    X_test = np.r_[test_doc_codes.values()]
    Y_test = [adjust_label(test_doc_labels[i]) for i in test_doc_codes]
    ...
    if args.multilabel_clf:
        ...
        Y_test = Y[-n_test:]
        print [ (i, encoder.classes_[i]) for i in range(len(encoder.classes_) )]
        encoder_classes = encoder.classes_.tolist()
        encoder_classes.remove('16') # who knows why 16 goes missing
    ...
    if args.multilabel_clf:
        ...
        results = multiclass_classifier(X_train, Y_train, X_val, Y_val, \
                X_test, Y_test, nb_epoch=args.n_epoch, batch_size=args.batch_size, seed=seed, target_names=encoder_classes)
